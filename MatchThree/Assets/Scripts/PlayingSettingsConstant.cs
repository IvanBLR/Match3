using System.Numerics;

public class PlayingSettingsConstant
{
    public static readonly string GAME_FIELD_ROW = "GAME_FIELD_ROW";
    public static readonly string GAME_FIELD_COLUMN = "GAME_FIELD_COLUMN";
    public static readonly string PLAYING_SET = "PLAYING_SET";
    public static readonly string SOUND_ON_OFF = "SOUND_ON_OFF";
    public const float SCREEN_WIDTH = 6.7f;
    public static readonly Vector3 START_GRID_POSITION = new(-3.375f, -4.7f, 0);
    public const int SCREEN_ROW = 1080;
    public const int SCREEN_COLUMN = 1920;
    public const float DELAY = 0.1f;
    public const float MIN_CLICK_INTERVAL = 0.33f;
    public static readonly string BEST_SCORE = "BEST_SCORE";
}